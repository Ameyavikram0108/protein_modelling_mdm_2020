...
def Vp(p,K,d):
    ...
    the vector K
    """
    return K[0] + sum(abs(p-K[1:N+1])**gamma @ K[N*(gamma-1) + 1 : N*gamma + 1] for gamma in range(2,d+1))
...
ax.scatter(E,V_vals,'rx')
xx = np.linspace(min(V_vals),max(V_vals),1000)
ax.plot(xx,xx,'--k')
ax.set_xlabel(r"$E(\mathbf{p})$")
ax.set_ylabel(r"$V(\mathbf{p})$")
ax.set_title(r"""Scatter plot of predicted energy values against measured energy
